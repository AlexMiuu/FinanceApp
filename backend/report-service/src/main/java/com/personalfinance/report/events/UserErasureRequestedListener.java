...
public class UserErasureRequestedListener {
     ...
     * mirrors expense-service's UserErasureRequestedListener.
     */
    ...
    public void onUserErasureRequested(Events.UserErasureRequested event) {
        log.info("Erasing report data for user {}", event.userId());
        try {
            privacyService.eraseUserData(event.erasureRequestId(), event.userId());
        } catch (Exception e) {
            log.error("Failed to erase report data for user {} (erasureRequestId {}) — erasure request stays PENDING",
